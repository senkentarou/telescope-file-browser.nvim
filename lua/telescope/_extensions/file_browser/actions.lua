...
fb_actions.goto_parent_dir = function(prompt_bufnr, bypass)
  ...
  local current_picker = action_state.get_current_picker(prompt_bufnr)
  local finder = current_picker.finder
  local current_dir = Path:new(finder.path)
  local parent_dir = current_dir:parent():absolute()
  ...
  fb_utils.selection_callback(current_picker, current_dir .. Path.path.sep)
  current_picker:refresh(finder, { reset_prompt = true, multi = current_picker._multi })
end
